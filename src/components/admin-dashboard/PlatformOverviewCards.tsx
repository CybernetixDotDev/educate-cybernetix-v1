...
export function PlatformOverviewCards({ overview }: { overview: AdminOverview }) {
  const cards: Card[] = [
    { label: "Students", value: overview.totalStudents, helper: "Learner profiles", tone: "bg-teal-50 text-teal-900" },
    { label: "Parents", value: overview.totalParents, helper: "Parent accounts", tone: "bg-emerald-50 text-emerald-900" },
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-7">
      {cards.map((card) => (
        <div key={card.label} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className={`mt-3 inline-flex rounded-2xl px-3 py-2 text-3xl font-black ${card.tone}`}>{card.value}</p>
